...
<head>
    ...
    <title>Add Category - TechHive</title>
...
<body class="bg-[#FDFDFC] text-[#1b1b18] min-h-screen">
    <header class="w-full border-b border-[#e3e3e0] bg-white mb-6">
        <div class="max-w-4xl mx-auto p-6 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-[#706f6c]">Create a new category</p>
            </div>
    ...
    <main class="max-w-4xl mx-auto px-6 pb-10">
        ...
        <form action="{{ route('categories.store') }}" method="POST" class="bg-white border border-[#e3e3e0] rounded-lg p-6 shadow-[0px_0px_1px_0px_rgba(0,0,0,0.03),0px_1px_2px_0px_rgba(0,0,0,0.06)] space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    class="w-full px-4 py-2 border border-[#e3e3e0] rounded-sm focus:ring-2 focus:ring-[#1b1b18] focus:outline-none">{{ old('description') }}</textarea>
            </div>
