                ...
                <div class="flex-1 space-y-3">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            ...
                            @if($order->customer)
                                <p class="text-xs text-white/60">{{ $order->customer->first_name }} {{ $order->customer->last_name }} • {{ $order->customer->email }}</p>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('seller.orders.show', $order) }}" class="inline-flex items-center px-4 py-2 text-sm bg-indigo-500 text-white rounded-md border border-indigo-600 hover:bg-indigo-600 transition">
                            View Details →
                        </a>
                        @if($order->status !== 'delivered' && $order->status !== 'cancelled')
                            <form action="{{ route('seller.orders.update-status', $order) }}" method="POST" onsubmit="return confirm('Mark order #{{ str_pad($order->id, 6, '0', STR_PAD_LEFT) }} as delivered?')">
                                @csrf
                                <input type="hidden" name="status" value="delivered">
                                <button type="submit" class="px-3 py-2 text-xs border border-green-300/60 text-green-100 hover:border-green-200 rounded-sm bg-green-500/10">
                                    Complete
                                </button>
                            </form>
                        @endif
