...
@php
    ...
    $subtitle = 'Update vendor profile details';
@endphp
    ...
    <div class="space-y-6 max-w-3xl">
        ...
        <div class="bg-white/5 border border-white/10 rounded-2xl shadow-lg shadow-black/30 p-6 space-y-6">
            <div>
                ...
                <h2 class="text-2xl font-semibold text-white">{{ $supplier->supplier_name }}</h2>
                <p class="text-sm text-white/60">Adjust contact and logistics information.</p>
            </div>
            ...
            <form action="{{ route('suppliers.update', $supplier) }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="address" rows="3"
                        class="w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/15 text-white placeholder-white/40 focus:ring-2 focus:ring-amber-400 focus:outline-none">{{ old('address', $supplier->address) }}</textarea>
                </div>
